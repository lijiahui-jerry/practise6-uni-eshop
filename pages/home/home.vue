<template>
  <block>
    ...
    <swiper class="home-swiper" duration="1000" interval="2000" indicator-dots autoplay circular>
      <swiper-item class="home-swiper-item" v-for="item in swiperList" :key="item.goods_id">
    ...
    <view class="navs">
      <view class="nav" v-for="(item, i) in navList" :key="i" @click="navClickHandler(item)">
      ...
      <view class="floor-item" v-for="(item, index) in floorList" :key="index">
        ...
        <view class="floor-img-container">
          <navigator class="left-img-container" :url="item.product_list[0].url">
            <image class="" :src="item.product_list[0].image_src"
              :style="{ width: item.product_list[0].image_width + 'rpx' }" mode="widthFix" />
          </navigator>

          <view class="right-container">
            <navigator class="right-img-container" v-for="(i, index2) in item.product_list" :key="index2" :url="i.url">
              <image :src="i.image_src" :style="{ width: i.image_width + 'rpx' }" mode="widthFix" class=""
                v-if="index2 !== 0" />
            </navigator>
          </view>
        </view>
