      ...
      <scroll-view class="left-scroll-view" scroll-y :style="{ height: wh + 'px' }">
        <block v-for="(item, index) in categoryList" :key="item.cat_id">
          <view class="left-scroll-view-item" :class="index === active ? 'active' : ''"
            @click="changeCurrentIndex(index)">{{ item.cat_name }}</view>
        </block>
      </scroll-view>
      <scroll-view class="right-scroll-view" scroll-y :style="{ height: wh + 'px' }">
        <view class="right-lv2" v-for="item in categoryList2" :key="item.cat_id">
          <view class="right-lv2-title">/~ {{ item.cat_name }} ~/</view>

          <view class="right-lv3">
            <navigator :url="'../../subpkg/goodsList/goodsList?cid=' + item2.cat_id" class="right-lv3-item"
              v-for="item2 in item.children" :key="item2.cat_id">
